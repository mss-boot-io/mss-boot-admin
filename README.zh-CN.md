...
mss-boot 是面向 Agent 的管理系统基础设施。**v1.3.5 是不可变的部分发布列车，不是
完整可用的采用者发行版。** Framework、Admin、Admin Web 与 Root Tag 已公开，但 Root
Release 和工具、Docs、公开 npmjs 包未发布。不得删除、移动、重建或复用任何 v1.3.5
身份。
...
v1.3.5 没有受支持的安装器、空目录应用创建、本地初始化或发行版升级流程。原候选接口已
从当前入门文档删除，避免被误认为可下载命令。下一个未使用的发行版本尚未选择。
