---
name: mss-thin-host
description: Safely develop and validate this generated MSS v1.3.3 Thin Host while keeping exact Admin Distribution dependencies and business-owned extensions separate.
---

# MSS Thin Host

1. Read `AGENTS.md`, `.mss/project.yaml`, `.mss/capabilities.yaml`, and the affected `.mss/modules/*.yaml` or `.mss/features/*.yaml`.
2. Use the installed release binaries. Confirm `mss --version` is v1.3.3 before generation or upgrade work; never compile a private local CLI as the adopter path.
