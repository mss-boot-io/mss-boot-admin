...
func TestApplyChangesRejectsReplacedManagedPreimageBeforeAnyMutation(t *testing.T) {
	for _, action := range []Action{ActionUpdate, ActionDelete} {
		t.Run(string(action), func(t *testing.T) {
			...
			const createPath = "admin/modules/supplier/a_generated.go"
			const targetPath = "admin/modules/supplier/z_generated.go"
			if err := os.MkdirAll(filepath.Join(root, "admin", "modules", "supplier"), 0o755); err != nil {
				t.Fatalf("MkdirAll() error = %v", err)
			}
			plannedPreimage := []byte("// Code generated by mss old. DO NOT EDIT.\npackage supplier\nconst generatedVersion = 1\n")
			if err := os.WriteFile(filepath.Join(root, filepath.FromSlash(targetPath)), plannedPreimage, 0o644); err != nil {
				t.Fatalf("WriteFile(planned preimage) error = %v", err)
			}

			create, err := compareOutput(repository, output{
				path: createPath, content: []byte("// Code generated by mss create. DO NOT EDIT.\npackage supplier\n"),
				managed: true, fileMode: 0o644,
			})
			if err != nil {
				t.Fatalf("compareOutput(create) error = %v", err)
			}
			var target Change
			if action == ActionUpdate {
				target, err = compareOutput(repository, output{
					path: targetPath, content: []byte("// Code generated by mss updated. DO NOT EDIT.\npackage supplier\nconst generatedVersion = 2\n"),
					managed: true, fileMode: 0o644,
				})
			} else {
				var exists bool
				target, exists, err = compareObsoleteOutput(repository, targetPath)
				if err == nil && !exists {
					t.Fatal("compareObsoleteOutput() did not plan delete")
				}
			}
			if err != nil {
				t.Fatalf("plan %s error = %v", action, err)
			}
			if target.Action != action {
				t.Fatalf("planned action = %s, want %s", target.Action, action)
			}

			concurrent := []byte("// Code generated by mss concurrent. DO NOT EDIT.\npackage supplier\nconst generatedVersion = 99\n")
			if !sameGeneratedArtifactKind(plannedPreimage, concurrent) {
				t.Fatal("test replacement is not another legal generated artifact")
			}
			if err := os.WriteFile(filepath.Join(root, filepath.FromSlash(targetPath)), concurrent, 0o644); err != nil {
				t.Fatalf("WriteFile(concurrent replacement) error = %v", err)
			}

			err = applyChanges(repository, []Change{create, target})
			var conflict *ConflictError
			if !errors.As(err, &conflict) || conflict.Path != targetPath {
				t.Fatalf("applyChanges(%s) error = %v", action, err)
			}
			if _, err := os.Stat(filepath.Join(root, filepath.FromSlash(createPath))); !errors.Is(err, os.ErrNotExist) {
				t.Fatalf("preimage conflict allowed an earlier create: %v", err)
			}
			after, err := os.ReadFile(filepath.Join(root, filepath.FromSlash(targetPath)))
			if err != nil || !bytes.Equal(after, concurrent) {
				t.Fatalf("concurrent generated artifact = %q, %v", after, err)
			}
		})
	}
}

func TestApplyChangesRejectsReplacedManagedUnchangedPreimageBeforeAnyMutation(t *testing.T) {
	root := t.TempDir()
	repository, err := os.OpenRoot(root)
	if err != nil {
		t.Fatalf("OpenRoot() error = %v", err)
	...
	const createPath = "admin/modules/supplier/a_generated.go"
	const updatePath = "admin/modules/supplier/b_generated.go"
	const unchangedPath = "admin/modules/supplier/z_generated.go"
	if err := os.MkdirAll(filepath.Join(root, "admin", "modules", "supplier"), 0o755); err != nil {
		t.Fatalf("MkdirAll() error = %v", err)
	}
	updatePreimage := []byte("// Code generated by mss update-old. DO NOT EDIT.\npackage supplier\nconst updateVersion = 1\n")
	unchangedPreimage := []byte("// Code generated by mss stable. DO NOT EDIT.\npackage supplier\nconst stableVersion = 1\n")
	for path, content := range map[string][]byte{
		updatePath:    updatePreimage,
		unchangedPath: unchangedPreimage,
	} {
	...
	update, err := compareOutput(repository, output{
		path: updatePath, content: []byte("// Code generated by mss update-new. DO NOT EDIT.\npackage supplier\nconst updateVersion = 2\n"),
		managed: true, fileMode: 0o644,
	})
	if err != nil {
		t.Fatalf("compareOutput(update) error = %v", err)
	}
	unchanged, err := compareOutput(repository, output{
		path: unchangedPath, content: unchangedPreimage, managed: true, fileMode: 0o644,
	})
	if err != nil {
		t.Fatalf("compareOutput(unchanged) error = %v", err)
	}
	if create.Action != ActionCreate || update.Action != ActionUpdate || unchanged.Action != ActionUnchanged {
		t.Fatalf("planned actions = %s/%s/%s", create.Action, update.Action, unchanged.Action)
	}

	concurrent := []byte("// Code generated by mss concurrent. DO NOT EDIT.\npackage supplier\nconst stableVersion = 99\n")
	if !sameGeneratedArtifactKind(unchangedPreimage, concurrent) {
		t.Fatal("test replacement is not another legal generated artifact")
	}
	if err := os.WriteFile(filepath.Join(root, filepath.FromSlash(unchangedPath)), concurrent, 0o644); err != nil {
		t.Fatalf("WriteFile(concurrent replacement) error = %v", err)
	}

	err = applyChanges(repository, []Change{create, update, unchanged})
	var conflict *ConflictError
	if !errors.As(err, &conflict) || conflict.Path != unchangedPath {
		t.Fatalf("applyChanges(unchanged replacement) error = %v", err)
	}
	if _, err := os.Stat(filepath.Join(root, filepath.FromSlash(createPath))); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("unchanged preimage conflict allowed an earlier create: %v", err)
	}
	if after, err := os.ReadFile(filepath.Join(root, filepath.FromSlash(updatePath))); err != nil || !bytes.Equal(after, updatePreimage) {
		t.Fatalf("unchanged preimage conflict allowed an earlier update: %q, %v", after, err)
	}
	if after, err := os.ReadFile(filepath.Join(root, filepath.FromSlash(unchangedPath))); err != nil || !bytes.Equal(after, concurrent) {
		t.Fatalf("concurrent unchanged artifact = %q, %v", after, err)
	}
}
