...
func TestGenerateEmbeddedWorksOutsideGitAndIsIdempotent(t *testing.T) {
	...
	if !plan.DryRun || !plan.Success || plan.Identities.Foundation.Version != "1.3.5" || plan.Identities.Foundation.Commit != strings.Repeat("a", 40) {
	...
	for _, expected := range []string{
		"releases/download/v1.3.5/install-mss.sh",
		"--version v1.3.5",
		"mss doctor --strict",
		"mss verify --all",
	} {
	...
	if !strings.Contains(string(lockData), "resolution: {tarball: http://127.0.0.1:") ||
		!strings.Contains(string(lockData), "/artifacts/frozen-admin-web-1.3.5.tgz, integrity: sha512-") {
	...
	if !strings.Contains(string(goModuleData), "github.com/mss-boot-io/mss-boot-admin/mss-boot v1.3.5 // indirect") {
...
func TestGenerateEmbeddedRejectsIncompleteBuildProvenance(t *testing.T) {
	...
	buildinfo.Version, buildinfo.Commit, buildinfo.Timestamp = "v1.3.5", "deadbeef", "2026-08-25T12:34:56Z"
...
func TestUpgradeEmbeddedUsesMatchingSourceAndPreservesBusinessFiles(t *testing.T) {
	...
	options := UpgradeOptions{
		...
		RequestedDistributionVersion: "v1.3.5",
	...
	if !plan.DryRun || !plan.Success || plan.FoundationRoot != "embedded://mss/1.3.5" {
...
func embeddedFrontendRegistry(t *testing.T) string {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		_, _ = fmt.Fprintf(
			...
			integrity,
			"http://"+request.Host+"/artifacts/frozen-admin-web-1.3.5.tgz",
		)
	}))
