...
- Before a coordinated Framework version exists publicly, PR and feature-freeze Admin checks use the repository `go.work` plus the repository-external consumer script with confined temporary replacements. After Framework publication, the protected Admin release workflow must resolve and test the exact public Framework version with `GOWORK=off`; never weaken that publication gate or commit a local `replace`.
