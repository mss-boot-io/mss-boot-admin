...
CORE_CURRENT_FILES = (
    ...
    Path("docs/docs/releases/v1-3-3.md"),
)
...
ALLOWED_RELEASE_ROOT = {"index.md", "v1-3-3.md"}
...
def upgrade_contract_errors(root: Path) -> list[str]:
    ...
    for path in UPGRADE_CONTRACT_FILES:
        ...
        upgrade_commands = re.findall(
            r"mss upgrade admin (?:v1\.3\.3|__MSS_DISTRIBUTION_VERSION__)",
            text,
        )
        ...
        for pattern in required_bases:
            if not re.search(pattern, text, re.MULTILINE):
                errors.append(
                    ...
                    f"v1.3.3-qualified tag and immutable digest ({pattern})"
                )
    else:
...
def collect_errors(root: Path = ROOT) -> list[str]:
    ...
    if version != "v1.3.3":
        errors.append(
            f"documentation contract is for v1.3.3, release policy declares {version}"
        )
    ...
    if not archive.is_dir():
        ...
        for page in sorted(archive.glob("v*.md")):
            ...
            if "v1.3.3" not in text or "/getting-started" not in text:
                errors.append(
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print("current documentation contract OK: v1.3.3 package-first, links and archive")
    return 0
