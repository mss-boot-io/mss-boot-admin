...
for command_name in mss mss-mcp; do
  ...
  cp /dev/null "${command_path}"
  {
    ...
    printf 'if [[ "${1:-}" == "--version" ]]; then echo "%s version v1.3.6 (commit %s, timestamp %s)"; exit 0; fi\n' "${command_name}" "${commit}" "${timestamp}"
    echo 'exit 0'
  } > "${command_path}"
  chmod 0755 "${command_path}"
done
printf 'version=v1.3.6\ncommit=%s\ntimestamp=%s\n' "${commit}" "${timestamp}" > "${fixture}/archive/BUILD-INFO"
printf 'test-only license fixture\n' > "${fixture}/archive/LICENSE"
asset="mss-tools-v1.3.6-${platform}-${architecture}.tar.gz"
tar -czf "${fixture}/${asset}" -C "${fixture}/archive" BUILD-INFO LICENSE mss mss-mcp
"${checksum_command[@]}" "${fixture}/${asset}" | sed "s#${fixture}/##" > "${fixture}/SHA256SUMS.tools-v1.3.6"
