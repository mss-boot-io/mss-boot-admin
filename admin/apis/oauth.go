...
func (e *User) OAuthAuthorize(c *gin.Context) {
	...
	api.OK(&dto.OAuthAuthorizeResponse{AuthorizeURL: authorizeURL, ExpiresAt: issued.ExpiresAt})
}
...
func (e *User) oauthCallback(c *gin.Context) {
	...
	result, err := e.codeExchange()(c, req.Provider, req.Code)
	...
	result.Provider = string(req.Provider)
	result.Intent = string(record.Intent)
	api.OK(result)
}
