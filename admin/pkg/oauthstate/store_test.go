...
func TestMemoryStateIsHighEntropyBoundAndOneTime(t *testing.T) {
	store := New()
	record := Record{Provider: "github", Intent: IntentBinding, UserID: "user-1", SessionID: "session-1"}
	...
	if consumed.Provider != record.Provider || consumed.Intent != record.Intent || consumed.UserID != record.UserID || consumed.SessionID != record.SessionID {
