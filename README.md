...
selected complete package-first release candidate, but it is not public,
adoptable, or stable.** v1.3.5 and v1.3.6 are immutable partial trains. No
identity from either train may be deleted, moved, recreated, reused, or
completed.
...
and `npm-auto`. Until every public surface reconciles from that commit, no
v1.3.7 download, install, creation, or upgrade procedure is supported.
...
adoptable before publication and does not complete v1.3.5 or v1.3.6.
