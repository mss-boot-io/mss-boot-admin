...
| Agent tooling | `.github/workflows/agent-native-ci.yml` | Agent contracts, governance, installers, and cross-platform portability remain available by explicit dispatch; local verification is the normal development and release-quality path. |
| Frontend | `.github/workflows/frontend-v6-ci.yml` | Biome, TypeScript, Vitest, package, production build, delivery smoke, and browser evidence remain available by explicit dispatch; the same suite is authoritative locally through `mss verify --all`. |
...
- Feature branches do not run duplicate `push` workflows. Pull requests retain only fast required contexts and ordinary Go smoke tests. Complete qualification is local; Agent and frontend extended qualification runs only when explicitly dispatched, while publication workflows continue to own release tags.
...
- Manual Agent infrastructure qualification does not rebuild documentation content. Shared Agent and release contracts continue to use their own root paths.
...
The workflow file `.github/workflows/ci.yml` keeps the workflow name `CI` and exposes a final job named `admin-ci`. The `main` branch rule requires that exact context, so the workflow is intentionally unfiltered for pull requests. A pull request runs only the ordinary Admin test behind that aggregate; race, coverage, vet/tidy, workspace compatibility, and standalone binary work move to the complete local verifier. Agent and frontend extended qualification are manual-only diagnostics, not automatic post-merge audits or pull-request gates. Other broad component workflows may remain path-scoped to merged `main` or explicit dispatch. Release tags are owned only by their publication workflows; the self-bound local `mss verify --all --release-evidence --expect-commit <sha>` report owns broad release quality, while the successful candidate preview builds and verifies only the exact publication artifacts reused by the tag workflows.
...
- Agent and frontend extended workflows remain available as explicitly dispatched diagnostics; they do not automatically mark a merged `main` commit red and are not a prerequisite for artifact staging.
...
- Agent and frontend extended workflows have only `workflow_dispatch`, retain their complete job matrices, and are not required pull-request checks;
- other broad component workflows have no `pull_request` trigger and remain reachable from merged `main` plus explicit dispatch;
