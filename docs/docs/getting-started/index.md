...
nav:
  title: 快速开始
  order: 1
description: 不克隆 Foundation 源码，使用 v1.3.5 工具和公开包创建可开发、可验证、可升级的 Thin Host
keywords: [v1.3.5 package first mss thin host quick start]
---
...
:::warning
v1.3.5 公开对账完成前，本页是候选操作合同，不是已可下载的证明。请先在
[发布页](/releases/v1-3-5)确认 Root Release、Go/npm 包、镜像和 Docs 都绑定同一
merged-main 提交；任一项缺失都不要执行安装或升级，也不要改用源码克隆绕过门禁。
:::
...
`SHA256SUMS.tools-v1.3.5`。它不会请求 `sudo`，也不会修改 profile。正式支持的公开
工具只有 `mss` 与 `mss-mcp`。
...
手工拼装或丢失 manifest 的仓库不能直接套用升级；先用 v1.3.5 在新目录生成基线，再按
业务所有权迁入规格和业务文件并重新验证，不要伪造 manifest。

匹配 v1.3.5 工具的只读计划不需要 Foundation 源码：
...
[v1.3.5 发布合同](/releases/v1-3-5)；不要回退到源码克隆路径伪装成使用方验证。
