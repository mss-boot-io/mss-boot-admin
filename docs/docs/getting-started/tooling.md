...
keywords: [v1.3.5 mss mss-mcp install checksum tools]
---

# v1.3.5 工具说明

v1.3.5 候选只规划对外发布两个工具；Root GitHub Release 完成公共对账后，以下资产
才成为采用者入口：
...
`mss-tools-v1.3.5-{linux|darwin}-{amd64|arm64}.tar.gz` 或 Windows zip。
...
默认版本同样固定为 v1.3.5，默认目录位于当前用户目录。显式参数更适合可复现脚本。

:::warning
不要用 `go install` 代替上述 Release 安装器。两个带精确 `@v1.3.5` 的公共命令包只用于
...
1. 识别受支持的系统和架构；
2. 下载一个精确版本工具包；
3. 读取 `SHA256SUMS.tools-v1.3.5`；
4. 校验归档摘要；
5. 安装 `mss` 与 `mss-mcp`；
6. 保留用户对 PATH 的控制。

两个二进制都应通过 `--version` 报告 v1.3.5 与 Release 源提交。摘要、版本或提交身份
不一致时应停止，而不是继续安装。
