...
nav:
  title: 发布
  order: 3
description: 计划中的 v0.8.0 部署失败后的 forward-fix、代码回退与完整数据恢复合同
keywords: [v0.8.0 rollback restore forward-fix recovery]
---

# v0.8.0 回滚与恢复

v0.8.0 包含安全清理与身份语义收紧，不能把“换回旧二进制”当作完整回滚。默认策略是 **停止写入并 forward-fix**；只有满足恢复条件时才回到升级前完整快照。
...
| v0.8 已产生业务写入 | forward-fix | 直接恢复旧快照会丢失新写入，需明确停机/RPO 决策 |
| 仅前端失败 | 恢复上一静态制品并清 HTML/CDN 缓存 | 保留 v0.8 后端与数据库，不回滚数据 |
...
只回退二进制/前端而保留 v0.8 数据库，必须先在隔离恢复环境证明前一版本能够：
...
8. 经风险确认后逐步恢复流量，并安排重新执行 v0.8 升级。

不要把 v0.8 产生的新业务行手工复制回旧库，除非存在单独评审的数据迁移计划。
...
恢复成功不等于 v0.8 已发布。只有重新完成 [发布合同](/releases/v0-8-0) 全部门禁后才能再次进入 stable 发布流程。
