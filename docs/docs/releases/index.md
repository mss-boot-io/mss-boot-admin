...
> 当前唯一活动发布目标是 **v1.2.3**。根、Framework 和唯一的 Ant Design V6
...
- [v1.2.3 发布、升级与回滚合同](/releases/v1-2-3)

## v1.2.2 component-partial

当前状态：**组件部分发布 / 已由 v1.2.3 接续**。Framework 与唯一 V6 前端已公开；根
候选资格和 `v1.2.2` 标签已完成，但根公开镜像在 QEMU 内编译 arm64 2320 秒，并在
40 分钟上限处于镜像清单导出/推送阶段被取消，因此根镜像和 GitHub Release 未完成。
所有公开引用保持不可变。

- [v1.2.2 不完整列车审计记录](/releases/v1-2-2)

## v1.2.1 component-partial

当前状态：**组件部分发布 / 先由 v1.2.2 接续，当前活动目标为 v1.2.3**。Framework 与唯一 V6 前端已公开；根
`v1.2.1` 标签和 runtime image 已创建，但根 Release 工作流仍上传原始 V6 目录并因含
`:` 的生成路径失败，六个平台 ZIP 和根 GitHub Release 未发布。所有公开引用保持不可变。

- [v1.2.1 不完整列车审计记录](/releases/v1-2-1)

## v1.2.0 component-partial

当前状态：**组件部分发布 / 先由 v1.2.1、v1.2.2 接续，当前活动目标为 v1.2.3**。`mss-boot/v1.2.0` 已公开；
`web/antd-v6/v1.2.0` 标签已创建，但制品上传失败，因此没有 V6 构建制品、镜像或
GitHub Release；根 `v1.2.0` 标签从未创建。所有已有标签与证据保持不可变，不能移动、
复用或补发；v1.2.1 与 v1.2.2 均未完成根 Release，完整同步发行现改用 v1.2.3。
