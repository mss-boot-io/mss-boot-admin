...
> 当前唯一活动发布目标是 **v1.2.2**。根、Framework 和唯一的 Ant Design V6
...
- [v1.2.2 发布、升级与回滚合同](/releases/v1-2-2)

## v1.2.1 component-partial

当前状态：**组件部分发布 / 已由 v1.2.2 接续**。Framework 与唯一 V6 前端已公开；根
...
当前状态：**组件部分发布 / 先由 v1.2.1 接续，当前活动目标为 v1.2.2**。`mss-boot/v1.2.0` 已公开；
`web/antd-v6/v1.2.0` 标签已创建，但制品上传失败，因此没有 V6 构建制品、镜像或
GitHub Release；根 `v1.2.0` 标签从未创建。所有已有标签与证据保持不可变，不能移动、
复用或补发；v1.2.1 仍未完成根 Release，完整同步发行现改用 v1.2.2。
