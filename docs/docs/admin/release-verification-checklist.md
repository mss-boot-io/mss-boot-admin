...
在本地完成 `mss verify --all` 与内置浏览器验收，并通过 PR 必需检查；合并后，在精确且
tracked-clean 的 merged-main commit 上固定完整本地报告，唯一 Root preview 只生成和核验
...
- [ ] Foundation 根目录在精确且 tracked-clean 的 merged-main commit 上执行 `mss verify --all` 成功，前后完整 SHA 一致
...
- [ ] PR、审查记录或仓库外发布台账记录完整 commit、`trackedClean: true`、精确命令、verify report 哈希或摘要及退出结果；本地报告单独存在不授权发布
