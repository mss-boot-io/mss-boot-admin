...
title: 容器发布状态与部署合同
order: 6
description: v1.3.6 候选镜像合同、v1.3.5 部分发布事实与 Thin Host 部署边界
---
...
为 release candidate，但后端和前端镜像尚未发布。本页只描述 source-only 与候选部署合同；
公共镜像对账前，v1.3.6 不可采用，也不开放拉取或部署命令。
:::
...
v1.3.6 候选生成的 Dockerfile 从同一协调版本的公共依赖和冻结锁构建后端、Admin Web
