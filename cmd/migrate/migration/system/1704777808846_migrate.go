...
func init() {
	...
	migration.Migrate.SetVersion(migration.GetFilename(fileName), _1704777808847Migrate)
}

func _1704777808847Migrate(db *gorm.DB, version string) error {
	err := models.InitTenant(db)
	if err != nil {
		return err
	}
	return db.Transaction(func(tx *gorm.DB) error {
		...
		c.Request.Header.Set("Referer", "http://localhost:8000/")
		tx = tx.WithContext(c)
