...
func _1746193492486Migrate(db *gorm.DB, version string) error {
	return db.Transaction(func(tx *gorm.DB) error {
		...
		err := tx.
			Exec(`CREATE UNIQUE INDEX idx_user_config ON mss_boot_user_configs(user_id, name, "group")`).
			Error
		if err != nil {
			return err
		}
		...
		err = tx.
			Exec(`CREATE UNIQUE INDEX idx_app_config ON mss_boot_app_configs(name, "group")`).
			Error
		if err != nil {
			return err
		}
