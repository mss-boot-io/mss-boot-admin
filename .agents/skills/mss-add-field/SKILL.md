---
name: mss-add-field
description: Add or safely adjust a string, enum, or boolean field in an existing generated MSS module and regenerate its v1.3.3 contracts. Trigger for a simple field, validation, index, filter, or form change. Do not use for relations or destructive schema evolution.
---
...
- A Thin Host uses the installed `mss` v1.3.3 binary and `.mss/modules/<module>.yaml`.
...
- The public v1.3.3 frontend generator contract for this workflow is limited to non-relational `string`, `enum`, and `bool` fields with `ownership.mode: none`.

If the change needs a relation, nullable/representation transition, import surface, workflow, row scope, destructive rename/drop, or automatic live-data backfill, stop and create a separate Feature specification plus handwritten migration/business code. The v1.3.3 module generator does not make that upgrade safe by itself.
