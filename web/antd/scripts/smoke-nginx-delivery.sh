...
image_name="mss-boot-admin-web-smoke:local-$$"
container_id=""
...
for _attempt in $(seq 1 30); do
  ...
  if [[ -n "${published_port}" ]] && curl --silent --fail "http://127.0.0.1:${published_port}/index.html" >/dev/null; then
    break
  fi
  sleep 1
done
...
if [[ -z "${published_port}" ]]; then
  echo "Unable to resolve the Nginx smoke-test port" >&2
  exit 1
fi

entry_asset="$(find dist -maxdepth 1 -type f -name 'umi.*.js' -printf '%f\n' | sort | head -n 1)"
if [[ -z "${entry_asset}" ]]; then
  echo "Unable to find the content-hashed Umi entry asset" >&2
  exit 1
fi
...
echo "Nginx delivery smoke test passed"
