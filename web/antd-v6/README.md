...
`web/antd-v6` is the sole Admin browser application in this repository. It is
an independently built React 19 and Ant Design 6.6.0 application with its own
lockfile, immutable image, `web/antd-v6/v{version}` tag namespace, deployment,
and previous-V6 rollback history. Retired frontend source and protocols are not
build, runtime, or recovery inputs.
...
- deterministic generated modules, with Supplier as the golden generator
  fixture but no procurement menu in a fresh or upgraded reference database.
...
frontend target and writes only under `web/antd-v6`.
